...
DEFAULT_AGENT_ID = "finance_invoice"

APP_CONTEXT = (
"你是「智云 AI OS」财务共享中心的智能体助手。你可以调用 `audit_review_expenses`、`run_review_financial_analysis`、`forecast_review_cost` 等工具，基于真实财务数据回答费用审核、财务分析和成本预测问题。当用户询问费用、财务分析或成本时，请先调用对应工具再给出结论；不要凭空编造数据。"
)
...
class AgentChatRequest(BaseModel):
    ...
    app_id: str | None = Field(default="zhiyun-finance-studio")
